...
function actionToQMK(type, action) {
  ...
  if (type === 'midi_cc' || type === 'midi_note') return 'KC_NO' // handled by process_record_user
...
function normalizePreset(preset) {
  ...
  for (let li = 0; li < km.length; li++) {
    ...
    for (const [matrix, entry] of Object.entries(layer)) {
      ...
      // skip normalization for MIDI types — they use structured action strings
      if (entry.type === 'midi_cc' || entry.type === 'midi_note') continue
...
function generateKeymapC(device, preset) {
  ...
  // Build layer color section
  const layerColors = layers.map((l, i) =>
    `//   layer ${i}: ${l.name} — ${l.color || '#ffffff'}`
  ).join('\n')

  // Collect MIDI CC entries across all layers for custom handler
  const midiCcEntries = []
  for (let li = 0; li < numLayers; li++) {
    const layerKeys = keymap[li] || {}
    for (const [matrix, entry] of Object.entries(layerKeys)) {
      if (entry && entry.type === 'midi_cc' && entry.action) {
        const m = {}
  ...
  // Build custom code sections
  let customCode = ''

  // MIDI CC handler
  if (midiCcEntries.length > 0) {
    const ccCases = midiCcEntries.map(e =>
      `        if (record->event.key.row == ${e.row} && record->event.key.col == ${e.col} && layer == ${e.layer}) {\n` +
      `            midi_send_cc(&midi_device, ${e.ch}, ${e.cc}, pressed ? ${e.val} : 0);\n` +
      `            return false;\n` +
      `        }`
    ).join('\n')
    customCode += `
...
`
  ...
  if (presetFeatures.rgb && layers.some(l => l.color)) {
    const cases = layers.map((l, i) => {
...
void raw_hid_receive(uint8_t *data, uint8_t length) {
    switch (data[0]) {
        case 0x01: rgb_matrix_set_color(data[1], data[2], data[3], data[4]); break;
        case 0x02: rgb_matrix_set_color_all(data[1], data[2], data[3]); break;
        case 0xFF: rgb_matrix_reload_from_eeprom(); break;
    }
